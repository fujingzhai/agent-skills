---
name: yt-dlp
description: 视频平台内容处理套件。用户给 YouTube 或B站视频链接并要求整理文稿、逐字稿、字幕、总结、摘要、概括、要点、访谈实录或导出文稿时使用；即使用户只说"总结"或"摘要"，也必须按全文文稿流程处理（开头写摘要、正文保留全文），严禁只写一篇总结稿。优先找第一方现成文稿，否则下最佳字幕、清洗、翻译并导出 DOCX；B站支持复用 Arc/Chrome 登录态提取人工 CC 与平台 AI 字幕；无字幕时停止，不自动 ASR。开工前必须先运行 yt check 检查依赖。
---
...
## 铁律：用户说“总结”也要出全文，不是只写总结稿

用户经常只说“帮我总结这个视频”或“给我这个视频的摘要”。**这仍然是文稿整理任务**：
按子技能的完整流程走（先探测来源 → 找现成稿 → 下载清洗 → 校错翻译 → 成稿），
产出的是**开头有 500 字摘要、正文是完整逐字稿**的文稿；摘要只是成稿的一部分。

只有用户明确限定“只要一段 300 字摘要、不要全文”时，才可以只写摘要。拿不准就出全文。
